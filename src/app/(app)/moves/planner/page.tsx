...
export default function LocationPlannerPage() {
  ...
  const selectClass = "border border-gray-300 rounded-md px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-brand-forest/30 focus:border-brand-forest";
    ...
    <div className="space-y-5">
      ...
      {filtered && (
        <div className="flex gap-3 flex-wrap items-center">
          <div className="bg-brand-forest/5 border border-brand-forest/10 px-4 py-2 rounded-lg">
            <span className="text-brand-forest font-semibold text-sm">{filtered.summary.total}</span>
                ...
                <CardContent className="pt-0">
                  ...
                  {shouldCollapse && (
                    <button
                      ...
                      className="text-xs text-brand-forest hover:underline font-medium mt-2 w-full text-center py-1"
                    >
                      {isExpanded
                        ? "Show less"
                ...
                <div className="divide-y">
                  {filtered.unlocated.map((horse) => (
                    <div key={horse.id} className="py-2">
                      <Link
                        href={`/horses/${horse.id}`}
                        className="text-sm font-medium text-gray-600 hover:text-brand-forest hover:underline"
                      >
                        {horse.name}
                      </Link>
                      <span className="text-xs text-gray-400 font-mono ml-2">
