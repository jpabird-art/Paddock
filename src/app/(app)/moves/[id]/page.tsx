  ...
  return (
    <div className="max-w-4xl space-y-6">
      <div className="flex items-center gap-2 text-sm text-gray-500">
        <Link href="/moves" className="hover:text-brand-forest hover:underline">
          Horse Moves
        </Link>
        <span>/</span>
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 mb-2">
            {move.name ? (
              <>
                {move.name} —{" "}
                <Link href={`/horses/${move.horse.id}`} className="text-brand-forest hover:underline">
                  {move.horse.name}
                </Link>
              </>
            ) : (
          ...
          <div className="divide-y">
            ...
            {siblingMoves.map((s) => (
              <div key={s.id} className="flex items-center gap-3 py-2">
                <Link
                  href={`/moves/${s.id}`}
                  className="font-medium text-brand-forest hover:underline text-sm"
                >
                  {s.horse.name}
                </Link>
