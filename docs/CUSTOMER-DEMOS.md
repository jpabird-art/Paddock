...
7. Seed each EMPTY demo database once using the command below. The script refuses existing application data (apart from anonymous failed-sign-in audits, which it preserves), mismatched organisation names, missing confirmation, or an existing credentials output file. It never sends email. Do not enable demo seeding on normal customer deployments or make it part of startup.
